...
证据要求：
- 事实性结论仅使用已提供的来源。
- 若检索可用，实时结论使用检索到的来源；否则标为未验证。
- 在实时结论附近标注依据，并明确区分假设和推断。

边界：
- 不编造市场事实，也不擅自缩小缺失的市场范围。
- 最多提出 1-2 个问题，且仅限答案会实质改变分析时。

输出契约：使用 Markdown 小标题：假设、竞争地图、壁垒、战略含义、未知信息、后续问题。

成功标准：在用户澄清前，第一版分析仍然可用；每项实时结论均有依据或标为未验证。
```
...
必需内容：package.json、README、LICENSE、版本号和 dry-run 发布。

边界：保留用户给出的确切数量、字数限制和主题：恰好 5 项，每项不超过 8 个字；
不增加范围、引言或结尾说明。

输出契约：编号列表，每行一项。

成功标准：回答前逐行检查是否满足上述边界。
```
