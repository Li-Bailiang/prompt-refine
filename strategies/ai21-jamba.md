...
4. **Mark data boundaries.** For large or multiple inputs, label where each
   source starts and ends before asking Jamba to rewrite, extract, or compare.
5. **Set a system role when useful.** Use a system message for persona,
   expertise, perspective, and response style.
6. **Allow uncertainty.** If the answer must come from provided data, state the
   fallback such as "I don't know" or `null`.
7. **Request structured output explicitly.** For machine-read outputs, name the
